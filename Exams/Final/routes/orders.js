...
// User order history
router.get('/order-history', isAuth, async (req, res) => {
  try {
    const orders = await Order.find({ user: req.session.user.id }).sort({ orderDate: -1 }).lean();
    res.render('order-history', { orders });
  } catch (error) {
    console.error('Error fetching order history:', error);
    res.render('order-history', { orders: [], error: 'Failed to load order history.' });
  }
});
